...
# This sequence contains positions from minimum to maximum
# and then middle again.  Move the motor to each position in order.
for x in xrange(30, 80, 5):
	position = (x * (max_position - min_position) / 100) + min_position
	...
	time.sleep(.5)
position = (50 * (max_position - min_position) / 100) + min_position
duty_cycle_percentage = position * 100 / ms_per_cycle
pan_pwm.start(duty_cycle_percentage)
time.sleep(.5)
pan_pwm.stop()
